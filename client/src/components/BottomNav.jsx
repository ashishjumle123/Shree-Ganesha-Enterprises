...
export default function BottomNav() {
    ...
    return (
        <nav className="md:hidden fixed bottom-0 left-0 w-full h-[60px] bg-white shadow-[0_-4px_10px_rgba(0,0,0,0.1)] z-50 flex justify-around items-center border-t">
            <Link to="/" className={`flex flex-col items-center w-1/4 ${getStyle('/')}`}>
                <Home className="w-6 h-6 mb-1" />
                <span className="text-[11px] font-medium leading-none">Home</span>
            </Link>

            <Link to="/wishlist" className={`flex flex-col items-center w-1/4 ${getStyle('/wishlist')}`}>
                <Heart className="w-6 h-6 mb-1" />
                <span className="text-[11px] font-medium leading-none">Wishlist</span>
            </Link>

            <Link to="/cart" className={`flex flex-col items-center w-1/4 ${getStyle('/cart')} relative`}>
            ...
            {user ? (
                <Link to="/profile" className={`flex flex-col items-center w-1/4 ${getStyle('/profile')}`}>
                    <User className="w-6 h-6 mb-1" />
                    <span className="text-[11px] font-medium leading-none">Profile</span>
                </Link>
            ) : (
                <Link to="/login" className={`flex flex-col items-center w-1/4 ${getStyle('/login')}`}>
                    <User className="w-6 h-6 mb-1" />
                    <span className="text-[11px] font-medium leading-none">Profile</span>
                </Link>
            )}
